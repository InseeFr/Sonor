      ...
      const promises = interviewers.map(
        interv =>
          new Promise(resolve2 => {
            this.service.getStateCountByInterviewer(survey.id, interv.id, date, data => {
              const interviewer = {};
    ...
    if (mode === BY_SURVEY_ONE_INTERVIEWER) {
      ...
      if (mode !== BY_SITE) {
        ...
        if (getDataForSingleSurvey) {
          ...
          surveysToGetInterviewersFrom = await this.getFormattedCampaignsForMainScreen(
            date,
            null,
            campaigns
          );
  ...
  async getDataForCollectionTableBySite(survey, date, cb) {
    Promise.all([
      ...
      cb({
        linesDetails: lines,
        total: {
          france: Utils.formatForCollectionTable({}, data[0].france, data[1].france),
        },
      });
    });
  }
    ...
    this.service.getInterviewersByCampaign(survey.id, interviewers => {
      ...
      const p2 = new Promise(resolve => {
        ...
        Promise.all(promises2).then(data => {
          ...
          const totalFrance = Utils.formatForCollectionTable({}, data[1].france, data[2].france);
          resolve({
            total: {
              dem: totalDem,
              france: totalFrance,
      ...
      const promises = campaigns.map(
        c =>
          new Promise(resolve => {
            const datas = [
              { survey: c.label },
              this.service.getContactOutcomesByInterviewer(c.id, interviewer.id, date),
              this.service.getStateCountByInterviewer(c.id, interviewer.id, date),
            ];
      ...
      const promises = interviewers.map(
        interviewer =>
          new Promise(resolve => {
            const datas = [
              { interviewer },
              this.service.getClosingCausesByInterviewer(survey.id, interviewer.id, date),
            ];
            Promise.all(datas).then(data => {
              resolve(
                data[1] !== null ? Utils.formatForProvisionalStatusTable(data[0], data[1]) : null
              );
            });
          })
      );
      ...
      const promises = campaigns.map(
        c =>
          new Promise(resolve => {
            const datas = [
              { survey: c.label },
              this.service.getClosingCausesByInterviewer(c.id, interviewer.id, date),
            ];
            Promise.all(datas).then(data => {
              resolve(
                data[1] !== null ? Utils.formatForProvisionalStatusTable(data[0], data[1]) : null
