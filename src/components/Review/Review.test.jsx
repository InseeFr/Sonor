...
const mockFinalizeSurveyUnits = jest.fn(listSU =>
  Promise.resolve(listSU.map(su => (su === '4818' ? { status: 500 } : { status: 200 })))
);
...
const TestingRouter = ({ ComponentWithRedirection }) => (
  <Router history={history}>
    <Switch>
      <Route
        path="/review/:id?"
        render={routeProps => <ComponentWithRedirection {...routeProps} />}
      />
      <Route
        path="*"
        render={routeProps => (
...
it('Component is correctly displayed (for one survey)', async () => {
  ...
  const component = render(
    <TestingRouter
      ComponentWithRedirection={props => (
        <Review location={{ survey }} match={props.match} dataRetreiver={mockDataRetreiver} />
      )}
    />
  );
...
it('Component is correctly displayed (for all surveys)', async () => {
  ...
  const component = render(
    <TestingRouter
      ComponentWithRedirection={props => (
        <Review location={{}} match={props.match} dataRetreiver={mockDataRetreiver} />
      )}
    />
  );
  // Should match snapshot (rows displayed)
...
it('Select 2 survey units and validate', async () => {
  ...
  await wait(() =>
    expect(mockSuccess).toHaveBeenLastCalledWith(
...
it('Select a survey unit and validate (error response)', async () => {
  ...
  await wait(() =>
    expect(mockError).toHaveBeenLastCalledWith(`${D.reviewAlertError}: 4818.`, D.error, 3500)
  );
});
...
it('Click on the 3 cells of a SU line', async () => {
  ...
  const questionnaireButton = component.baseElement
    .querySelectorAll('tbody')[0]
    .querySelectorAll('.fa-calendar')[0];
