...
it("Select another survey", async () => {
  ...
  expect(screen.getByTestId("Redirect-url").innerHTML).toEqual(
    `\"${redirectUrl}\"`
  );
...
it("Select 2 survey units and validate", async () => {
  ...
  const firstCheckbox = component.baseElement.querySelector(
    'input[value="1032"]'
  );
  const secondCheckbox = component.baseElement.querySelector(
    'input[value="4819"]'
  );
...
it("Check and uncheck all SUs", async () => {
  ...
  const checkAllBox = component.baseElement.querySelector(
    'input[name="checkAll"]'
  );
...
it("Close modal with cross", async () => {
  ...
  await wait(() =>
    expect(
      component.baseElement.querySelector(".modal-dialog")
    ).not.toBeTruthy()
  );
});
