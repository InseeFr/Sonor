...
function makeTableForExport(data) {
  ...
  return header.concat(
    data.map((line) => [
      ...
      line.id,
      line.displayName,
      `${line.interviewer?.interviewerLastName ?? ""} ${
        line.interviewer?.interviewerFirstName ?? ""
      }`,
      line.ssech,
...
class CloseSUTable extends React.Component {
    ...
    this.state = {
      ...
      checkboxArray:
        props.data?.map((element) => {
          return { id: element.id, isChecked: false };
        }) ?? [],
      checkAll: false,
      show: false,
      stateModified: "",
  ...
  handleCheckAll(e) {
    ...
    const newCheckboxArray = Utils.handleCheckAll(
      e.target.checked,
      checkboxArray,
      displayedLines,
      pagination
  ...
  toggleCheckBox(id) {
    ...
    const { newCheckboxArray, newCheckAll } = Utils.getOnToggleChanges(
      id,
      checkboxArray,
      displayedLines,
      pagination
  ...
  updateLines(matchingLines) {
    ...
    const newCheckboxArray = checkboxArray.map((element) => {
      return { id: element.id, isChecked: false };
    });
    this.setState({
      checkboxArray: newCheckboxArray,
                ...
                >
                  <thead>
                    <tr>
                      ...
                      <th
                        onClick={handleSortFunct("campaign")}
                      ...
                      <th
                        onClick={handleSortFunct("identificationState")}
                        className="Clickable ColIdentificationState"
                      >
                        {D.identificationState}
                        <SortIcon val="identificationState" sort={sort} />
                      </th>
                      <th
                        onClick={handleSortFunct("contact_outcome")}
                        className="Clickable ColLocation"
                      >
                        {D.contactOutcome}
                        <SortIcon val="contact_outcome" sort={sort} />
                      </th>
                      <th
                        onClick={handleSortFunct("questionnaire_state")}
                  ...
                  <tbody>
                    {displayedLines
                      ...
                      .map((line) => {
                        const element = checkboxArray.find(
                          (checkbox) => checkbox.id === line.id
                        );
                        return (
                          <CloseSurveyUnitLine
                            key={line.id}
